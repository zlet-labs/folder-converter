...
Based on empirical benchmarking across 15 synthetic and structural fixtures on Windows x64, the verdict is **USE DOCLING SELECTIVELY** as an isolated out-of-process conversion worker for Zlet Converter.

### Key Justifications:
1. **Unrivaled Layout & Structural Fidelity:** Docling successfully resolved multi-column reading order (`F02`), converted complex tables to GitHub-Flavored Markdown (`F03`), eliminated running headers and footers as furniture artifacts (`F04`), dehyphenated broken line wraps (`F05`), and recognized rasterized text through built-in RapidOCR without external Tesseract dependencies (`F06`).
2. **Sub-second Office & HTML Ingestion:** Office documents (DOCX `F08`: 0.16s, PPTX `F09`: 0.06s, XLSX `F10`: 0.03s) and HTML (`F11`: 0.03s) convert without heavyweight ML pipeline overhead and without requiring Microsoft Office COM automation.
3. **100% Permissive Licensing:** Docling code is MIT-licensed, its layout model (`docling-layout-heron`) is Apache-2.0, its table model (`tableformer`) is CDLA-Permissive-2.0, and OCR is Apache-2.0. No AGPL or OpenRAIL-M restrictions exist.
4. **Verified Offline Privacy:** Socket-level monitoring proved zero external network requests during offline execution (`HF_HUB_OFFLINE=1`), ensuring zero data leakage.
5. **Why "SELECTIVELY" rather than blanket adoption:** PDF deep-learning conversion demands substantial storage (~566 MB model cache, ~1.17 GB virtual environment) and ~1.0–1.3 GB peak RSS with 2–13s per-page CPU execution time. Consequently, Docling should be deployed as an **isolated, optional worker process** with on-demand model acquisition rather than bundled directly into the base lightweight WPF MSI installer.
...
| `F02` | `F02_multicolumn.pdf` | PDF | Two-column academic layout; reading order integrity across columns |
...
| `F06` | `F06_scanned.pdf` | PDF (Raster) | Pure image-only PDF; OCR activation and character recognition fidelity |
| `F07` | `F07_cyrillic.pdf` | PDF | Russian Cyrillic glyph decoding, UTF-8 integrity |
| `F08` | `F08_structured.docx` | DOCX | Native DOCX parsing: headings, nested lists, table, hyperlink |
...
| `F12` | `F12_nested_lists.pdf` | PDF | Multi-tier ordered and unordered nested lists |
...
| `F15` | `F15_mixed_unicode.pdf` | PDF | Trilingual document: Latin (diacritics), Cyrillic, CJK (SimSun) |
...
2. **Arm B — Docling 2.126.0 (Target Candidate):**
   ...
   - Native document tree export (`DoclingDocument`) and canonical Markdown export.
3. **Alternative Parser Assessment (Marker & PyMuPDF4LLM):**
   - *PyMuPDF4LLM:* Evaluated but **disqualified** due to AGPL-3.0 license restrictions which conflict with Zlet Converter's MIT license.
...
### PDF Findings
- **Multi-Column Reading Order (`F02`):** Docling successfully recognized the physical column boundary, reading Column 1 completely before moving to Column 2. In contrast, simple text extractors frequently risk line interleaving.
- **Table Structure Extraction (`F03`):** Docling reconstructed a perfect GitHub-Flavored Markdown table with headers and data rows. Baseline extraction collapsed the table into unstructured whitespace-separated text without markdown pipe syntax.
...
### OCR Findings (`F06`)
- On an image-only PDF containing no text streams, Docling's integrated `RapidOCR` triggered automatically on CPU.
- Extracted 448 characters with 100% accuracy, including reference codes (`SC-98421`) and complete prose.
...
- In the mixed trilingual fixture (`F15`), Latin diacritics (`café`, `über`), Russian Cyrillic (`Привет мир`), and Chinese CJK (`测试中文文档解析能力: 你好世界`) were preserved simultaneously in valid UTF-8 Markdown.

### DOCX, PPTX, XLSX, HTML Applicability (`F08`–`F11`)
- **DOCX (`F08`):** Converted in **160 ms**. Headings (`##`, `###`), bullet lists (`- `), numbered lists (`1. `, `2. `), tables, and hyperlinks (`https://github.com/...`) were cleanly extracted.
- **PPTX (`F09`):** Converted in **62 ms**. Generated clean markdown slides with `#` slide headers, bullet points, and tables.
- **XLSX (`F10`):** Converted in **33 ms**. Each worksheet was formatted into a distinct markdown table.
- **HTML (`F11`):** Converted in **30 ms**. Converted standard HTML markup directly into markdown with hyperlinked anchors.
...
- **Empty Document (`F13`):** Docling processed the single blank page in 2.36s and emitted 0 characters cleanly without exception or null pointer errors.
- **Corrupted Document (`F14`):** When presented with invalid binary header data, Docling threw a structured `ConversionError` from `docling-parse` within 516 ms. It did not crash the host process or hang indefinitely.
...
|---------|-------------|--------------|-------------|-----------------|---------------|------------------|
| `F01` | Simple text PDF | 13.14s | 1034.0 MB | High (clean headings & flow) | 0.21s | Medium (raw text) |
| `F02` | Multi-column PDF | 2.27s | 1038.9 MB | High (column order intact) | 0.01s | Medium (raw text) |
| `F03` | Table PDF | 9.64s | 1018.1 MB | High (GFM markdown table) | 0.01s | Poor (unstructured text) |
| `F04` | Header/Footer PDF | 4.04s | 1115.4 MB | High (furniture stripped) | 0.01s | Poor (text polluted) |
| `F05` | Broken Wrap PDF | 2.21s | 1122.8 MB | High (dehyphenated) | 0.01s | Medium (broken lines) |
| `F06` | Scanned Image PDF | 6.02s | 1175.9 MB | High (100% OCR accuracy) | 0.00s | Failed (0 chars extracted) |
| `F07` | Cyrillic PDF | 2.36s | 1140.8 MB | High (clean UTF-8) | 0.01s | High (text extracted) |
| `F08` | Structured DOCX | 0.16s | 1142.1 MB | High (headings, lists, table) | 0.02s | Low (flat text lines) |
| `F09` | Slide Deck PPTX | 0.06s | 1147.8 MB | High (slides, tables, bullets) | 0.01s | Low (flat text) |
| `F10` | Multi-Sheet XLSX | 0.03s | 1148.0 MB | High (multi-table markdown) | 0.01s | Medium (pipe delimited) |
| `F11` | Semantic HTML | 0.03s | 1148.1 MB | High (GFM formatting & links) | 0.00s | Low (stripped tags) |
| `F12` | Nested Lists PDF | 7.15s | 1265.2 MB | High (hierarchical lists) | 0.01s | Medium (flat text) |
| `F13` | Empty PDF | 2.36s | 1271.2 MB | High (clean 0-byte output) | 0.00s | High (0-byte output) |
| `F14` | Corrupted PDF | 0.52s | 1271.2 MB | High (graceful ConversionError) | 0.00s | High (graceful PdfStreamError) |
| `F15` | Mixed Unicode PDF | 2.31s | 1285.8 MB | High (Latin + Cyrillic + CJK) | 0.01s | High (Unicode preserved) |

---

## Local & Offline Verification

Offline execution was validated through a strict isolation test:
1. Environment variables set: `HF_HUB_OFFLINE=1`, `TRANSFORMERS_OFFLINE=1`, `HTTP_PROXY=http://0.0.0.0:1`, `HTTPS_PROXY=http://0.0.0.0:1`.
2. Python `socket.socket.connect` was dynamically monkey-patched to record and abort any outgoing TCP connection attempt.
3. Tests executed: `F01_simple_text.pdf` (9.30s), `F06_scanned.pdf` OCR (4.47s), and `F08_structured.docx` (0.12s).
4. **Result:** All three conversions succeeded offline. Exactly **0 socket connections were attempted**.
5. **Verdict:** Docling performs 100% locally with zero cloud telemetry and zero document leakage.
...
- **Model Cache Locations:**
  - HuggingFace: `%USERPROFILE%\.cache\huggingface\hub\`
...
- **Copyleft / Incompatible Licenses:** **0 detected**. (Note: `fpdf2` is LGPL-3.0, but it is strictly a test fixture generator tool and is not a dependency of Docling).
- **Commercial Viability:** Fully compatible with Zlet Converter's MIT license and suitable for commercial distribution.
...
- **Cold Process Import Time:**
  - `import docling`: 0.06s.
  - `from docling.document_converter import DocumentConverter`: 6.15s.
  - `DocumentConverter()` instantiation: 0.10s.
  - Total process init: **~6.3s**.
- **Runtime Memory (Peak RSS):**
  - Office / HTML conversion: ~100–200 MB in isolation.
  - PDF layout and OCR conversion: **1018 MB – 1286 MB**.
- **Throughput:**
  - Non-PDF formats (DOCX, PPTX, XLSX, HTML): **10–30 documents/second**.
  - Digital PDFs: **0.4–0.5 pages/second** on 6-core Intel i5 CPU.
  - Scanned PDFs with OCR: **0.15–0.2 pages/second**.
...
- In Docling, `result.document.export_to_dict()` provides a `texts` list where each entry includes a `prov` block with page index and normalized bounding coordinates.
...
To achieve production-grade consistency across diverse converters, the post-processing Quality Layer in Zlet should handle:

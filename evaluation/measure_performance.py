...
# Measure Cold vs First vs Warm Conversion Footprint on F01
test_fixture = Path("evaluation/fixtures/F01_simple_text.pdf")
print("\n=== Cold Process vs In-Process First vs Warm Measurements (F01) ===")
print("1. Measuring isolated cold child process (process startup + import + init + first conversion)...")
cmd = [
    sys.executable,
    "-c",
...
print(f"   Cold Child Process:        {cold_dur:.3f} s | Process-Tree Peak RSS: {cold_peak_mb} MB")

print("2. Measuring in-process first conversion (loads model weights into memory)...")
current_pid = os.getpid()
first_peak_rss = 0
stop_event_first = threading.Event()

def first_monitor():
    global first_peak_rss
    while not stop_event_first.is_set():
        rss = sample_process_tree_rss(current_pid)
        if rss > first_peak_rss:
            first_peak_rss = rss
        stop_event_first.wait(0.01)

t_mon_first = threading.Thread(target=first_monitor, daemon=True)
t_mon_first.start()
t_first_start = time.perf_counter()
res_first = conv.convert(str(test_fixture))
md_first = res_first.document.export_to_markdown()
first_dur = time.perf_counter() - t_first_start
stop_event_first.set()
t_mon_first.join(timeout=1.0)
rss = sample_process_tree_rss(current_pid)
if rss > first_peak_rss:
    first_peak_rss = rss
first_peak_mb = round(first_peak_rss / (1024 * 1024), 2)
print(f"   In-Process First Conversion:{first_dur:.3f} s | Process Peak RSS:      {first_peak_mb} MB")

print("3. Measuring in-process warm conversion (weights already resident in memory)...")
warm_peak_rss = 0
stop_event_warm = threading.Event()

def warm_monitor():
    global warm_peak_rss
    while not stop_event_warm.is_set():
        rss = sample_process_tree_rss(current_pid)
        if rss > warm_peak_rss:
            warm_peak_rss = rss
        stop_event_warm.wait(0.01)

t_mon_warm = threading.Thread(target=warm_monitor, daemon=True)
t_mon_warm.start()
...
stop_event_warm.set()
t_mon_warm.join(timeout=1.0)
rss = sample_process_tree_rss(current_pid)
if rss > warm_peak_rss:
    warm_peak_rss = rss
warm_peak_mb = round(warm_peak_rss / (1024 * 1024), 2)
print(f"   In-Process Warm Conversion: {warm_dur:.3f} s | Process Peak RSS:      {warm_peak_mb} MB")
...
if summary_path.exists():
    ...
    for d in docling_runs:
        if d["fixture"] == "F01_simple_text.pdf":
            suite_f01_dur = d["duration_seconds"]
...
footprint_data = {
    ...
    "f01_conversion_benchmarks": {
        "cold_child_process_duration_seconds": round(cold_dur, 4),
        "cold_child_process_peak_rss_mb": cold_peak_mb,
        "in_process_first_conversion_duration_seconds": round(first_dur, 4),
        "in_process_first_conversion_peak_rss_mb": first_peak_mb,
        "in_process_warm_conversion_duration_seconds": round(warm_dur, 4),
        "in_process_warm_conversion_peak_rss_mb": warm_peak_mb,
        "suite_run_f01_duration_seconds": suite_f01_dur
    }
}
